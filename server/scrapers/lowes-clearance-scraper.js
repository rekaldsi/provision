...
async function scrapeSlickdeals() {
  ...
  const feeds = [
    "https://slickdeals.net/newsearch.php?mode=frontpage&searcharea=deals&q=lowe%27s+clearance&rss=1",
    "https://slickdeals.net/newsearch.php?mode=frontpage&searcharea=deals&q=lowes+penny+clearance&rss=1",
  ];
  ...
  for (const feedUrl of feeds) {
    try {
      ...
      console.error("[lowes-scraper] feed error:", err.message);
    }
  }
...
async function scrape() {
  try {
    ...
    console.error('[lowes-scraper] fatal error:', err.message);
    return [];
  }
}

scrape.scraperName = 'lowes-clearance';
module.exports = { scrape, scraperName: 'lowes-clearance' };
